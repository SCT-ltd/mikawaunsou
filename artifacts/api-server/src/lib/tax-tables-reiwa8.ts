 ...
 * @param standardRemuneration 報酬月額（standard_remuneration が設定されていればその値を渡す）
...
export function calculateSocialInsurance(
  ...
  const { hasPension } = getInsuranceGrade(standardRemuneration);
  ...
  const pension = hasPension ? Math.floor(pensionTotal / 2) : 0;
 ...
 */
/**
 * 令和7年（2025年）源泉徴収税額表（月額表・甲欄）の算出
 * 国税庁の「給与所得の源泉徴収税額表」に準拠した詳細ブラケット判定
 */
export function calculateIncomeTaxReiwa7(
  ...
  // 公式表より: 372k-375k の場合
  // 扶養0人: 13,540
  // 扶養1人: 10,220 (軽減額: 3,320)
  // 扶養2人: 8,380  (軽減額: 13,540 - 8,380 = 5,160)
  // 扶養3人: 6,540  (軽減額: 13,540 - 6,540 = 7,000)
...
/**
 * 令和8年（2026年）源泉徴収税額を計算する
 * 令和8年改正（基礎控除+10万、給与所得控除最低額+10万）を考慮
 */
export function calculateIncomeTaxReiwa8(
  ...
  if (X < 96_000) return 0; // 基礎控除増額分、非課税枠が広がる
