...
const RECORD_FIELDS = {
  ...
  sundayWorkDays: monthlyRecordsTable.sundayWorkDays,
  ...
  actualWorkHours: monthlyRecordsTable.actualWorkHours,
...
router.post("/monthly-records", async (req, res) => {
  ...
  if (existing.length > 0) {
    const [updated] = await db.update(monthlyRecordsTable).set({
      ...
      sundayWorkDays: body.sundayWorkDays ?? existing[0].sundayWorkDays,
      ...
      actualWorkHours: body.actualWorkHours ?? existing[0].actualWorkHours,
      updatedAt: new Date(),
  ...
  const [record] = await db.insert(monthlyRecordsTable).values({
    ...
    sundayWorkDays: body.sundayWorkDays ?? 0,
    notes: body.notes,
...
router.put("/monthly-records/:id", async (req, res) => {
  ...
  const [updated] = await db.update(monthlyRecordsTable).set({
    ...
    ...(body.sundayWorkDays !== undefined && { sundayWorkDays: body.sundayWorkDays }),
